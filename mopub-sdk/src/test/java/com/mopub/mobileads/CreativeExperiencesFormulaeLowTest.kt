...
class CreativeExperiencesFormulaeLowTest {

    companion object {
        // Non-rewarded
        const val CE_SETTINGS_LOW_NONREWARDED_VAST_STRING =
            """
                {
                    "hash": "12345",
                    "main_ad": {
        ...
        const val CE_SETTINGS_LOW_NONREWARDED_PLAYABLE_STRING =
            """
                {
                    "hash": "12345",
                    "main_ad": {
        ...
        const val CE_SETTINGS_LOW_NONREWARDED_DISPLAY_STRING = CE_SETTINGS_LOW_NONREWARDED_PLAYABLE_STRING

        // Rewarded
        const val CE_SETTINGS_LOW_REWARDED_DISPLAY_STRING =
            """
                {
                    "hash": "12345",
                    "main_ad": {
        ...
        const val CE_SETTINGS_LOW_REWARDED_PLAYABLE_STRING =
            """
                {
                    "hash": "12345",
                    "main_ad": {
        ...
        const val CE_SETTINGS_LOW_REWARDED_VAST_STRING = CE_SETTINGS_LOW_REWARDED_PLAYABLE_STRING
    }

    //region LOW CE Settings

    //region Non-rewarded 30s VAST - null (no) end card

    @Test
    fun `for low CE Settings for non-rewarded 30 second VAST with null end card`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_NONREWARDED_VAST_STRING)
    ...
    @Test
    fun `for low CE Settings for non-rewarded 30 second VAST with end card of type NONE`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_NONREWARDED_VAST_STRING)
        val ceSettings = CreativeExperienceSettingsParser.parse(ceSettingsJSONObject, false)
    ...
    @Test
    fun `for low CE Settings for non-rewarded 30 second VAST with end card of type STATIC`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_NONREWARDED_VAST_STRING)
        val ceSettings = CreativeExperienceSettingsParser.parse(ceSettingsJSONObject, false)
    ...
    @Test
    fun `for low CE Settings for non-rewarded 30 second VAST with end card of type INTERACTIVE`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_NONREWARDED_VAST_STRING)
        val ceSettings = CreativeExperienceSettingsParser.parse(ceSettingsJSONObject, false)
    ...
    @Test
    fun `for low CE Settings for non-rewarded 15 second VAST with null end card`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_NONREWARDED_VAST_STRING)
        val ceSettings = CreativeExperienceSettingsParser.parse(ceSettingsJSONObject, false)
    ...
    @Test
    fun `for low CE Settings for non-rewarded 15 second VAST with end card of type NONE`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_NONREWARDED_VAST_STRING)
        val ceSettings = CreativeExperienceSettingsParser.parse(ceSettingsJSONObject, false)
    ...
    @Test
    fun `for low CE Settings for non-rewarded 15 second VAST with end card of type STATIC`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_NONREWARDED_VAST_STRING)
        val ceSettings = CreativeExperienceSettingsParser.parse(ceSettingsJSONObject, false)
    ...
    @Test
    fun `for low CE Settings for non-rewarded 15 second VAST with end card of type INTERACTIVE`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_NONREWARDED_VAST_STRING)
        val ceSettings = CreativeExperienceSettingsParser.parse(ceSettingsJSONObject, false)
    ...
    @Test
    fun `for low CE Settings for non-rewarded playable with null end card`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_NONREWARDED_PLAYABLE_STRING)
        val ceSettings = CreativeExperienceSettingsParser.parse(ceSettingsJSONObject, false)
    ...
    @Test
    fun `for low CE Settings for non-rewarded display with null end card`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_NONREWARDED_DISPLAY_STRING)
        val ceSettings = CreativeExperienceSettingsParser.parse(ceSettingsJSONObject, false)
    ...
    @Test
    fun `for low CE Settings for rewarded display with null end card`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_REWARDED_DISPLAY_STRING)
    ...
    @Test
    fun `for low CE Settings for rewarded 30 second VAST with null end card`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_REWARDED_VAST_STRING)
        val ceSettings = CreativeExperienceSettingsParser.parse(ceSettingsJSONObject, true)
    ...
    @Test
    fun `for low CE Settings for rewarded 30 second VAST with end card of type NONE`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_REWARDED_VAST_STRING)
        val ceSettings = CreativeExperienceSettingsParser.parse(ceSettingsJSONObject, true)
    ...
    @Test
    fun `for low CE Settings for rewarded 30 second VAST with end card of type STATIC`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_REWARDED_VAST_STRING)
        val ceSettings = CreativeExperienceSettingsParser.parse(ceSettingsJSONObject, true)
    ...
    @Test
    fun `for low CE Settings for rewarded 30 second VAST with end card of type INTERACTIVE`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_REWARDED_VAST_STRING)
        val ceSettings = CreativeExperienceSettingsParser.parse(ceSettingsJSONObject, true)
    ...
    @Test
    fun `for low CE Settings for rewarded 15 second VAST with null end card`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_REWARDED_VAST_STRING)
        val ceSettings = CreativeExperienceSettingsParser.parse(ceSettingsJSONObject, true)
    ...
    @Test
    fun `for low CE Settings for rewarded 15 second VAST with end card of type NONE`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_REWARDED_VAST_STRING)
        val ceSettings = CreativeExperienceSettingsParser.parse(ceSettingsJSONObject, true)
    ...
    @Test
    fun `for low CE Settings for rewarded 15 second VAST with end card of type STATIC`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_REWARDED_VAST_STRING)
        val ceSettings = CreativeExperienceSettingsParser.parse(ceSettingsJSONObject, true)
    ...
    @Test
    fun `for low CE Settings for rewarded 15 second VAST with end card of type INTERACTIVE`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_REWARDED_VAST_STRING)
        val ceSettings = CreativeExperienceSettingsParser.parse(ceSettingsJSONObject, true)
    ...
    @Test
    fun `for low CE Settings for rewarded playable with null end card`() {
        val ceSettingsJSONObject = JSONObject(CE_SETTINGS_LOW_REWARDED_PLAYABLE_STRING)
        val ceSettings = CreativeExperienceSettingsParser.parse(ceSettingsJSONObject, true)
